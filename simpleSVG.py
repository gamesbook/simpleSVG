...
# Modifications by Derek R Hohls
...
#        self.scale() # after v.12, scale must be called explicitly, to prevent clipPath from premature definition
    ...
    def close(self):
        while self.group_count >= 1:
               self.group()
        endfile = "</svg>\n"
        self.svg.write(endfile)
        sys.stdout.write("The file %s was successfully written and closed by simpleSVG\n" % self.fname)
        self.svg.close()
        return
    ...
    def scale(self, xmin=0., xmax=1., ymin=0., ymax=1., #sets the user coordinates
        ...
        self.xscale = float(self.bbx - self.leftmarg - self.rightmarg) / \
                      (self.xmax - self.xmin)
        self.yscale = float(self.bby - self.botmarg - self.topmarg) / \
                      (self.ymax - self.ymin)
        clippath = """<defs><clipPath id="marginmask">
<rect x="%d" y="%d" width="%d" height="%d" />
</clipPath></defs>
""" % (
    self.leftmarg, self.topmarg, self.bbx-self.leftmarg-self.rightmarg,
...
#                        if (s): s+=", " #separate coordinate pairs by commas
                        if (s):
                            s += " " #no comma works in more browsers and software
                        x, y = bf[2 * n : 2 * n + 2]
...
#SIMPLE DRAWING
    ...
    def sector(self, x, y, r1, r2, a1, a2, **k):
        ...
        d = self.pathdata('M', x, y, 'm', hires(x21), hires(-y21),
                          ...
                          'l', hires(x12 - x22), hires(-y12 + y22),
                          'a', hires(r1), hires(r1), '0', largecircle + ',1',
                          hires(x11 - x12), hires(-y11 + y12), 'Z')
        self.path(d=d, style=style)
...
#COMPOSITE DRAWING
    ...
    def arrow(self, x1, y1, x2, y2, headsize, **k): #headsize is in pts
        ...
        self.path('M', hires(i2), hires(j2), 'L',
                  hires(i2 + headsize * ai),
                  hires(j2 + headsize * aj),
                  'L', hires( i2+ headsize * bi),
                  hires(j2 + headsize * bj), 'Z', stroke='none')
        self.group()
    ...
    def fatarrow(self,x1, y1, x2, y2, asize, **k):
        ...
        polypoints = [hires(q) for q in [
            i1 + v, j1 - u, i2 + v - u, j2 - u - v, i2, j2, i2 - v - u,
            j2 + u - v, i1 - v, j1 + u]]
...
#            p=[w,0,w-.5*d,.5*f]
    ...
    def image(self,x,y,file,**k):
        p = '<image x="%.2f" y="%.2f" xlink:href="%s" ' % \
            (self.ix(x), self.jy(y), file)
        for key in k.keys():
            p += key.replace('_', '-') + '="' + str(k[key]) + '" '
        self.svg.write(p + '/>\n')
...
### some functions independent of SVG class
...
def rgbstring(*colors):
    if colors:
        ...
        if isinstance(f, list) or isinstance(f, tuple):
            ...
            r, g, b = colors[0], colors[0], colors[0]
    else:
        r, g, b = 0, 0, 0
...
#    a.line(300,30023L,.5,.6,stroke="lime") # long integers (*100)  for hi-res coordinate are deprecated
    ...
    a.close()
    sys.stdout.write("The sample plot will now be displayed.\n")
    a.display()
